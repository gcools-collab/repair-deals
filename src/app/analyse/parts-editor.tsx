...
export default function PartsEditor({ identity, result, onResult }: Props) {
  ...
  function publish(nextRows: ManualRow[], baseResult: PartEstimateResult | null = result) {
    ...
    const automaticCandidates = plan.candidates.filter((candidate) => candidate.provider?.id === "ebay");
  ...
  function toggleAutomatic(id: string, selected: boolean) {
    ...
    onResult(aggregateSelectedParts(result, [...result.candidates.filter((candidate) => candidate.provider?.id === "ebay"), ...manualCandidates],
  ...
  const ebayCandidates = result?.candidates.filter((candidate) => candidate.provider?.id === "ebay") || [];

  return (
    <section className="parts-section">
      <div className="parts-heading">
        <div><p className="eyebrow">PARTS INTELLIGENCE</p><h2>Pièces nécessaires</h2></div>
        <button type="button" className="parts-prepare" disabled={!identity || loading} onClick={prepareSearches}>
          {loading ? "Recherche eBay…" : "Rechercher automatiquement"}
        </button>
      </div>
      ...
      {ebayCandidates.length > 0 && (
        <div className="ebay-candidates">
          ...
          {ebayCandidates.map((candidate) => (
            <article className="ebay-candidate" key={candidate.id}>
              ...
              <div className="ebay-candidate-main">
                ...
                <span>Compatibilité : {candidate.isCompatible === true ? "probable" : "à confirmer"} · confiance {candidate.compatibilityConfidence ?? 0}/100</span>
                {candidate.seller && <span>Vendeur : {candidate.seller}</span>}
              </div>
              <dl>
                <div><dt>Prix</dt><dd>{money(candidate.unitPrice, candidate.currency)}</dd></div>
                <div><dt>Livraison</dt><dd>{money(candidate.shippingCost, candidate.currency)}</dd></div>
                <div><dt>Total</dt><dd>{money(candidate.totalPrice, candidate.currency)}</dd></div>
              </dl>
              {candidate.url && <a href={candidate.url} target="_blank" rel="noreferrer">Voir sur eBay ↗</a>}
              <details><summary>Preuves et traçabilité</summary><ul>{candidate.evidence.map((item) => <li key={item}>{item}</li>)}</ul>
