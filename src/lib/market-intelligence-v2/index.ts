...
export function buildMarketQueryTiers(identity: ResolvedProductIdentity): MarketQueryTier[] {
  ...
  if (explicit) tiers.push({ tier: 1, query: [explicit, family].filter(Boolean).join(" "), requiredIdentityFields: ["confirmedReference"], confidence: 95, rationale: "Référence fabricant explicite" });
  const probable = identity.probableReferences.find((r) => r.confidence >= 80 && norm(r.reference) !== norm(explicit || ""));
  if (probable) tiers.push({ tier: 2, query: [probable.reference, family].filter(Boolean).join(" "), requiredIdentityFields: ["probableReferences", "family"], confidence: probable.confidence, rationale: "Référence probable fiable et famille" });
  const detailed = [brand, family, identity.screenSize, identity.year, identity.variant].filter(Boolean).join(" ");
  ...
  const model = [brand, family, identity.generation || identity.model, identity.screenSize, identity.variant].filter(Boolean).join(" ");
...
export function scoreMarketComparable(identity: ResolvedProductIdentity, listing: LeboncoinListing, acceptance = MARKET_V2_THRESHOLDS.acceptanceSimilarity): MarketComparableV2 {
  ...
  if (["accessory", "spare_part", "service", "lot"].includes(listing.listingKind)) exclusions.push(`Type d'annonce non comparable: ${listing.listingKind}`);
  ...
  if (/\b(coque|housse|cable|chargeur|manette|service|reparation|piece detachee)\b/.test(text)) exclusions.push("Accessoire, pièce ou service");
  let score = 0;
  ...
  if (has(text, identity.family || identity.model)) { score += 55; matches.push("Famille/modèle"); } else { conflicts.push("Famille/modèle absent"); score -= 25; }
  ...
  return { listing, similarityScore: score, identityMatches: matches, identityConflicts: conflicts, conditionSignals: conditions, excluded: exclusions.length > 0, exclusionReasons: [...new Set(exclusions)] };
}
